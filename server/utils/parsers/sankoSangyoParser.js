// ============================================
// SANKO SANGYO PARSER - UPDATED WITH 案件管理ID EXTRACTION
// ============================================
...
class SankoSangyoParser extends BaseParser {
  ...
  parse(csvData) {
    ...
    for (let i = 0; i < Math.min(csvData.length, 15); i++) {
      ...
      // Also check if row has 案件管理ID as a column header
      if (
        row['案件管理ID'] &&
    ...
    // ⚠️ Warning if no project ID found
    if (!defaultProjectId) {
    ...
    for (let i = dataStartIndex; i < csvData.length; i++) {
      ...
      if (rowProjectId) {
        ...
        finalProjectId = `MISSING_ID_${
          projectName || 'UNKNOWN'
        }_ROW${i}`.replace(/\s+/g, '_')
